...
- 🔄 **Полная синхронизация диалога**: сообщения, отправленные с телефона/десктопа,
  тоже появляются в VK в реальном времени (отправленные из VK не дублируются)
- 🖼 **Медиа в обе стороны** — фото, файлы, **голосовые** и **видео-кружки**
  (кружок в VK приходит проигрываемым видео — круглого формата в VK нет)
- 📝 **Расшифровка входящих голосовых** штатным движком ВКонтакте (без своего STT)
- 🧩 **Полный контекст при открытии**: подгружаются реальные медиа из свежих сообщений
- 📁 **Папки** Telegram · 🌳 **форумы** открываются по топикам
- ⭐ **Избранное** — только эти чаты шлют уведомления; при новых сообщениях
  прилетают **сами сообщения** (текст+медиа) с кнопкой «➡️ Перейти в диалог»
- ⚡ **Догоняющий опрос**: при входе в раздел Telegram бот сразу досылает всё,
  что накопилось в избранных чатах, пока вас не было
- 🗒 Телеграмное **«Избранное» (заметки, чат с собой)** доступно на чтение/запись
- 🔕 Исключение чатов из непрочитанных
- 🔔 **Уведомления по таймеру** (30с/1/5/10/15 мин)
...
- Список tmux-сессий (тап = подключиться + живой вывод, быстрое переключение)
- 🔀 **Гибридный вывод (VK)**: Claude/TUI (vim, htop) — «живой экран» правкой
  одного сообщения; обычная оболочка — **лента** (новый вывод новыми
  сообщениями, как терминальный scrollback). Бот сам определяет режим
- 🤖 **Claude Code** и 🧠 **DeepClaude** одной кнопкой
- 📂 **Мои проекты** (конфиг `tmux.projects`): тап по проекту → сессия
  стартует **сразу в его папке** + кнопки Claude / DeepClaude / Терминал.
  Удобно, когда правила Claude лежат в папке проекта
...
## 🧪 Тесты
```bash
.venv/bin/python tests.py      # 65 тестов
```
