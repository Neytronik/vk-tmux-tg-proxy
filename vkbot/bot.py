...
⏰ ПЛАНИРОВЩИК
  ...
  /tasks · /cancel <id>"""
    ...
    def _feed_delta(prev, cur):
        """Новые строки cur относительно prev.

        Два случая терминала:
        1) Экран НЕ прокрутился (обычный шелл): вывод дописывается снизу, у prev
           и cur общий ПРЕФИКС. При этом последняя строка prev — приглашение
           «$ » — превращается в «$ команда», поэтому допускаем расхождение
           ровно в последней строке prev.
        2) Экран прокрутился (заполнился): хвост prev совпадает с началом cur."""
        if not prev:
            return cur
        # (1) общий префикс
        p = 0
        for a, b in zip(prev, cur):
            if a != b:
                break
            p += 1
        if p >= len(prev) - 1:      # весь prev — префикс cur (кроме строки-приглашения)
            return cur[p:]
        # (2) прокрутка: хвост prev == начало cur
        max_ov = min(len(prev), len(cur))
        for k in range(max_ov, 0, -1):
            if prev[-k:] == cur[:k]:
                return cur[k:]
        return cur  # нет связи — экран сменился целиком
